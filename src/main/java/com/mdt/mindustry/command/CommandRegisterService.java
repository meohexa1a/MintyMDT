...
public final class CommandRegisterService {
    ...
    public void registerCommands(String group,
        ...
        if (registered.containsKey(group))
            unregister(group);
    ...
    public void unregister(String group) {
        ...
        pair.first().forEach(
                clientCommand -> clientCommand.prefixes().forEach(MintyMDTPlugin.getClientHandler()::removeCommand));
        pair.second().forEach(
                consoleCommand -> consoleCommand.prefixes().forEach(MintyMDTPlugin.getServerHandler()::removeCommand));
    }
    ...
    private void registerClient(ClientCommand cmd) {
        for (var prefix : cmd.prefixes())
            MintyMDTPlugin.getClientHandler().<Player>register(prefix, cmd.args(), cmd.description(),
                    (args, player) -> {
                        try {
